...
final class TrendChartAxisDaysTests: XCTestCase {
    ...
    func testNoTwoMarksEverFallOnTheSameDay() {
        for days in 1...400 {
            let marks = TrendChart.xAxisDays(spanning: series(days: days), calendar: calendar)
    ...
    func testASingleDayOfSamplesGetsExactlyOneMark() {
        let marks = TrendChart.xAxisDays(spanning: series(days: 1, perDay: 24), calendar: calendar)
    ...
    func testTheNewestDayIsAlwaysLabelled() {
        for days in 1...400 {
            ...
            let marks = TrendChart.xAxisDays(spanning: dates, calendar: calendar)
    ...
    func testEmptyAndSinglePointSeries() {
        XCTAssertTrue(TrendChart.xAxisDays(spanning: [], calendar: calendar).isEmpty)
        XCTAssertEqual(TrendChart.xAxisDays(spanning: [day0], calendar: calendar), [day0])
        XCTAssertEqual(TrendChart.xAxisDays(spanning: [day0], targetLabels: 0, calendar: calendar), [],
    ...
    func testAZoneThatShiftsAtMidnightKeepsEveryMarkOnItsDay() {
        ...
        for dayOffset in 0..<730 {
            ...
            for span in [6, 29, 89] {
                ...
                let marks = TrendChart.xAxisDays(spanning: [lo, hi], calendar: chile)
                for mark in marks {
    ...
    func testAnUnorderedSeriesSpansByItsExtremes() {
        ...
        XCTAssertEqual(TrendChart.xAxisDays(spanning: ordered.shuffled(), calendar: calendar),
                       TrendChart.xAxisDays(spanning: ordered, calendar: calendar))
    }
}

...
/// A hypnogram spelling wake `"awake"` (Oura's phase table, generic wearable JSON, or a stage-edited
/// night — the `edit_sleep_stages` vocabulary is `"awake"`, and its `stagelock` rows are section E's
/// human reference) had every wake epoch scored as SLEEP — biasing wake down and sleep up in precisely
/// the quantities this harness exists to measure.
...
///
/// `SleepStageVocabulary.canonicalStage` under a short local name. Nothing here is persisted — sleepbench
/// opens the database read-only and these arrays last one run — which is exactly the use the shared
/// canonicaliser permits: fold for comparison and bucketing, never write the folded string back.
func bucketLabel(_ label: String) -> String { SleepStageVocabulary.canonicalStage(label) }

...
final class AICoachEngine: ObservableObject {
    ...
    private func runAssistantStream(key: String,
        ...
        // Route through appendMessage so the streaming path honours the stored-transcript cap too (the
        // live UI path is sendStreaming, not send). Trimming drops from the FRONT, so the just-appended
        // bubble — found by id below, not by index — always survives.
        appendMessage(ChatMessage(id: assistantId, role: .assistant, text: ""))

...
struct TodayView: View {
    ...
    // Pomocná funkcia pre správne WHOOP zóny (67-100 zelená, 34-66 žltá, 0-33 červená)
        private func recoveryZoneColor(for score: Double) -> Color {
            switch score {
            case 67...:
                return .green
            case 34..<67:
    ...
    @ViewBuilder
        private func chargeRing(score: Double?, d: DailyMetric?, diameter: CGFloat) -> some View {
            if let s = score {
                // 1. Dynamically determine the color based on the score (WHOOP zones)
                let ringColor: Color = {
                    switch s {
                ...
                // 2. Pass this color directly into the GlowRing `color` parameter
                GlowRing(fraction: s / 100, value: s, format: { "\(Int($0.rounded()))" },
                         color: ringColor, diameter: diameter, lineWidth: diameter * 0.10)
                         
            } else if recoveryCalibration == nil, let carried = lastScoredCharge {
                let carriedColor: Color = {
                    switch carried.value {
                    case 67...: return .green
                    case 34..<67: return .yellow
                    default: return .red
                    }
                ...
                GlowRing(fraction: carried.value / 100, value: carried.value, format: { "\(Int($0.rounded()))" },
                         color: carriedColor, diameter: diameter, lineWidth: diameter * 0.10)
                    .opacity(0.8)
            } else {

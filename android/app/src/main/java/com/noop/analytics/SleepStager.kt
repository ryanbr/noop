...
object SleepStager {
    ...
    //
    // A brief mid-night get-up splits the night into separate stored sessions: the CENTERED 15-min
    // rolling stillness window smears an ~8-min bathroom trip past mergeMin into an "active" run (a
    // real MG night split at a 16-min seam), detection emits one stored session per still-run, and
    // nothing at the detection layer re-merges them — the #561/#861 bridges only group sessions
    // later, at selection/aggregation, so sub-minSleepMin sleep fragments between arousals are
    // dropped by the ladder before those bridges ever see them. When enabled (the live call site
    // threads PuffinExperiment.sleepWakeBridge, default OFF), [bridgeWakeGaps] assembles adjacent
    // sleep runs separated by ≤ [wakeBridgeMaxMin] into ONE candidate run between the run spine and
    // the gate ladder; the stager then scores the wake INSIDE the single session, and every
    // downstream gate still applies to the ASSEMBLED run. A merge that would assemble a span over
    // [maxMainSleepSpanS] is REFUSED (without that guard a pathological chain would assemble, then
    // be dropped WHOLE by the H4 cap — converting a partially-scored night into a zero-score night).
    //
     ...
     *  so selection/aggregation behaviour is unchanged for anything this bridge leaves split; well
    ...
    /**
     * Assemble adjacent sleep runs separated by at most [wakeBridgeMaxMin] minutes (whatever lies
     * between — an active run, or a plain data hole) into one candidate run; the intervening runs
     * are absorbed and the stager later scores the wake INSIDE the single session. A gap over the
     * threshold, a negative/overlapping gap (pinned `gap >= 0` semantics, matching
     * [bridgeSparseSleep]), or a merge whose assembled span would exceed [maxMainSleepSpanS] (the
     * H4 guard) never bridges. Pure + deterministic; mirrors Swift `bridgeWakeGaps`.
     */
    internal fun bridgeWakeGaps(
        ...
        for (p in periods) {
            ...
            if (p.stage == "sleep" && j >= 0 &&
                ...
                maxOf(out[j].end, p.end) - out[j].start <= maxMainSleepSpanS
            ) {
    ...
    fun detectSleep(
        ...
        // Wake-gap bridging (experimental, default OFF — see the section comment above
        // [wakeBridgeMaxMin]). When true, adjacent sleep runs across ≤ wakeBridgeMaxMin wake gaps
        // are assembled into one candidate run before the gate ladder, so a brief get-up stays
        // INSIDE one session as staged wake. The default keeps every existing caller byte-identical;
        // the live call site threads PuffinExperiment.sleepWakeBridge. Mirrors Swift.
        wakeBridge: Boolean = false,
        ...
        if (traceSink != null) {
            return detectSleepUncached(hr, rr, resp, gravity, tzOffsetSeconds, wristOff,
                bandSleepState, useSleepStagerV2, wakeBridge, sleepHRBaseline, traceSink)
        }
        val key = DetectKey(
        ...
        val sessions = detectSleepUncached(hr, rr, resp, gravity, tzOffsetSeconds, wristOff,
            bandSleepState, useSleepStagerV2, wakeBridge, sleepHRBaseline, null)
    ...
    private fun detectSleepUncached(
        ...
        // Wake-gap bridge (experimental, default OFF — see the section comment above
        // [wakeBridgeMaxMin]): assemble one candidate run per sleep period across short

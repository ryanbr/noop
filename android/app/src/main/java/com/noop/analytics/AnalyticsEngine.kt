...
object AnalyticsEngine {
    ...
    fun analyzeDay(
        ...
        // [SleepStager.wakeBridgeMaxMin]). When true, detection assembles one session per sleep
        // period across ≤ 45-min wake gaps, so a brief get-up stays inside one session as staged
        // wake. Default false keeps every pure-function caller/test byte-identical;
        // IntelligenceEngine threads PuffinExperiment.from(context).sleepWakeBridge. Mirrors Swift.

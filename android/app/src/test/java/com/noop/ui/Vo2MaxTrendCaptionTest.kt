...
class Vo2MaxTrendCaptionTest {
    ...
    fun `a single estimator throughout needs no caption`() {
        ...
        assertFalse(vo2MaxTrendHasMethodChange(nes))
    }

    @Test
    fun `a method change is captioned`() {
        assertTrue(vo2MaxTrendHasMethodChange(listOf(r("2026-08-01", "nes"), r("2026-08-08", "uth"))))
    }
    ...
    fun `a gap in days under one estimator is not a method change`() {
        ...
        assertFalse(vo2MaxTrendHasMethodChange(sparse))
    }
    ...
    fun `returning to the first estimator still counts as changed`() {
        ...
        assertTrue(vo2MaxTrendHasMethodChange(there_and_back))
    ...
    fun `degenerate inputs do not claim a change`() {
        assertFalse(vo2MaxTrendHasMethodChange(emptyList()))
        assertFalse(vo2MaxTrendHasMethodChange(listOf(r("2026-08-01", "nes"))))
    }
}

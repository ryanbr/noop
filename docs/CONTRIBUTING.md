...
- **Frame integrity is pinned by the same idiom.** Cross-platform agreement on *whether a frame is
  ...
  frame, the verdict, the reject reason **and** the historical-metadata classification. The file is
  generated by a standalone Swift run and taken verbatim; `FrameIntegrityOracleTests.swift` and
  `FrameIntegrityOracleTest.kt` each assert every row and all three fields, so changing a bound, a
  reason or a classification on one platform fails the build on the other. Extend that oracle when
  you touch the envelope rules, rather than adding a per-platform test that can only drift.
